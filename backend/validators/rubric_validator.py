...
[평가 관점]
- 5개 섹션 구조 (핵심개념·Bad/Good/Better·실전시나리오·체크리스트·요약) 준수. 도입부는 story 컴포넌트로 분리
- 핵심개념: 배경지식 → 한 줄 정의 → 원리(AI 메커니즘) → 핵심 요소 표 → 언제 쓰나 5블록이 있는지 확인
- `[ 더 좋은 프롬프트 만들기 ]` 소제목 정확히 포함
- **Bad → Good → Better 3단** (Good/Better/Best 아님) 실제 서술, `=== Bad ===` 마커만 허용 (괄호 설명 추가 금지)
- 소제목 앞 이모지 아이콘
- 마크다운 표 최소 1개 사용
- 핵심 개념에 카테고리별 전문 개념(SWOT 등) 명시
- 실전 시나리오 A·B·C가 주력 직무 + adjacent_jobs 2종으로 다양화되어 있고 상황별 복붙 템플릿 포함
- "뻔한 좋은 이야기" 배제 — 구체 인사이트·수치·실패담 포함
- story의 위인 실연을 원리 첫 줄에서 받아 프롬프트 기법으로 번역했는지 확인
...
[평가 관점]
- story_body가 350~600자에 가까운 자연스러운 한 덩어리 글이며 내부 마커가 노출되지 않음
- 위인의 실제 행동·사건이 구체 명사·연도·숫자·결과로 가시화됨
- 스토리가 해당 챕터 프롬프트 기법과 실질적으로 연결됨
- image_prompt가 행동·배경·구도·상황 중심이며 옷차림·생김새·표정 묘사가 없음
- 위인 직접 화법이 아니라 3인칭 격식 존댓말로 자연스럽게 쓰임
